(function (global) {
  ...
  var state = {
    ...
    channelIndex: 0,
    programIndex: 0,
    focus: 'programs' // days | channels | programs | detail
  };
  ...
  function defaultChannels() {
    return [
      { name: 'NTV', icon: '../../mocks/channel-poster-ntv.png', epg: 'Сёстры, 1-2 серия' },
      { name: 'Первый', icon: '../../assets/v4/img_default_channel.png', epg: 'Новости' },
      { name: 'Россия 1', icon: '../../assets/v4/img_default_channel.png', epg: 'Вечерний эфир' },
      { name: 'СТС', icon: '../../assets/v4/img_default_channel.png', epg: 'Шоу' }
    ];
  }
  ...
  function renderChannels() {
    ...
    for (var i = 0; i < state.channels.length; i++) {
      ...
      li.innerHTML =
        ...
        '<div class="ch-meta"><div class="ch-name">' + (ch.name || '') + '</div>' +
        '<div class="ch-epg">' + (ch.epg || '') + '</div></div>';
      ul.appendChild(li);
    }
  }
  ...
  function renderPrograms() {
    ...
    for (var i = 0; i < state.programs.length; i++) {
      ...
      var play = (p.hasRecord || p.activeRecord)
        ? '<svg class="prog-play" viewBox="0 0 24 24"><path fill="currentColor" d="M8 5v14l11-7z"/></svg>'
        : '<span class="prog-play"></span>';
      li.innerHTML =
  ...
  function syncHeader() {
    ...
    var logo = byId('iptv-header-logo');
    var name = byId('iptv-header-channel');
  ...
  var EpgUI = {
    applyDemo: function () {
      ...
      setVisible('iptv-program-loading', false);
    ...
    applyLoadingDemo: function () {
      this.applyDemo();
      setVisible('iptv-program-loading', true);
      setVisible('iptv-program-empty', false);
      var ul = byId('iptv-program-list');
      if (ul) ul.innerHTML = '';
    },
    ...
    setFocusedColumn: function (column) {
      if (column === 'days') state.focus = 'days';
      else if (column === 'channels') state.focus = 'channels';
      else if (column === 'programs') state.focus = 'programs';
      else if (column === 'detail') state.focus = 'detail';
      renderAll();
    },
